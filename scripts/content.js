...
const renderTree = new Event('renderTree');
...
function downloadObjectAsJson(exportObj, exportName) {
  ...
  var fileToSave = new Blob([JSON.stringify(exportObj, null, 4)], {
...
async function getContactInfo() {
  console.log('clicking contact btn!')
  document.querySelector("#top-card-text-details-contact-info").click();

  let refId = setInterval(() => {
    console.log('inside interval!')
    $(document).ready(function() {
      if (document.querySelector(".pv-profile-section.pv-contact-info")?.textContent?.toLowerCase().includes("info")) {
        const fetchedUrl = document.querySelector('.pv-contact-info__contact-type.ci-vanity-url a')?.href;
        url = fetchedUrl;
        const fetchedEmail = document.querySelector('.pv-contact-info__contact-type.ci-email a')?.href;
        const fetchedPhone = document.querySelector('.pv-contact-info__contact-type.ci-phone .t-14.t-black.t-normal')?.textContent?.trim();
        phone = fetchedPhone;
        if (fetchedEmail) {
          email = fetchedEmail.split(':')[1];
        }
        document.dispatchEvent(renderTree);
        console.log({email, phone, url})
        setTimeout(() => {
...
function main() {
  ...
  jsonview.render(tree, bodycontainer);
  ...
  document.addEventListener('renderTree', () => {
    data = extract();
    //alert(JSON.stringify(data));
    var bodycontainer = document.getElementById("slider").querySelector("#sbodycontainer");
    bodycontainer = bodycontainer.querySelector("#objectvalue")
...
function sliderGen() {
  ...
  var sliderDivInnerHTML = "\
    <div id='sheader'><h2>LinkedIn Profile Data Extractor</h2><hr/></div>\
...
function extract() {
  ...
  var locationElement = document?.querySelector("div.ph5 > div.mt2.relative > div.pb2.pv-text-details__left-panel > span.text-body-small.inline.t-black--light")
  ...
  for (const node of nodes) {
    ...
    const locationElement = node.querySelector('div.ph5 > div.mt2.relative > div.pb2.pv-text-details__left-panel > span.text-body-small.inline.t-black--light');
    var location = locationElement?.textContent || null;
  ...
  var experiences = UwU;
  const isWorking = experiences[0] && experiences[0][2]?.toLowerCase().includes("present");
  ...
  if (isWorking) {
    currentCompanyData.currentCompany = experiences[0][1];
    currentCompanyData.currentDesignation = experiences[0][0];
  }
  //extraction of experiences ends here//

  // extracting education section
  var nodes = $("#education ~ .pvs-list__outer-container > ul > li");
  let education = [];

  for (const node of nodes) {

    const schoolNameElement = node.querySelector('.t-bold');
    var schoolName = schoolNameElement?.textContent || null;
    schoolName = getCleanText(schoolName);

    const degreeNameElement = node.querySelector('.t-14.t-normal');
    ...
    const dateRangeElement = node.querySelector('.t-14.t-normal.t-black--light .visually-hidden');
    let dateRangeText = dateRangeElement?.textContent || null;
    dateRangeText = getCleanText(dateRangeText);

    const startDatePart = dateRangeText?.length && dateRangeText.split('-')[0];
    const startDate = startDatePart || null

    const endDatePart = dateRangeText?.length && dateRangeText.split('-')[1];
...
downloadBtn.addEventListener('click', function () {
  data = extract();
  downloadObjectAsJson(data, data?.name?.split(" ").join("_"))
})